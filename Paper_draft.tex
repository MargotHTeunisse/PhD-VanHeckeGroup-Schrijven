...
\end{figure}
In this section we discuss the crucial role of the ordering of the switching fields $U_i^{+, -}(S)$ for the topology of the corresponding transition graph. (...) Moreover, we discuss how the ordering of the switching fields provides an easy way to generate and label the transition graphs for non-interacting hysterons, while for interacting hysterons this is more challenging. (...)\\
To establish the importance of the ordering of the switching fields, we consider how a transition graph is constructed for a given set of switching fields. To start with, we ask which hysterons flip first from each state $S$, disregarding for the moment any avalanches that the initial hysteron flips might trigger\cite{grr}. To find the transitions from a state $S$, we vary the external driving $U$ until it reaches a value where a hysteron flips, which we will call $U^*$. There are two possible transitions at each state, corresponding to an increase or decrease in the driving $U$. For an increase in the driving $U$, the first hysteron to flip is that with the lowest up switching field, which we denote as $U^+(S)$:
...
Next we consider the additional steps that are required when $S^{(1)}$ is unstable. The system then enters into a new state $S^{(2)}$, for which we once again need to check the stability under the current driving. The current driving is still held at $U^*$, so the stability conditions are as in Equations \ref{eq:stableup} and \ref{eq:stabledown}, except that the state $S^{(1)}$ is now replaced by $S^{(2)}$. Thus, if the state $S^{(2)}$ is stable, we obtain a set of linear inequalities as in Equations \ref{eq:stableup} and \ref{eq:stabledown}. If the state $S^{(2)}$ is unstable, then the system moves to a third state $S^{(3)}$, and in this way the system can keep iterating through states until a stable state is reached. For each new state a corresponding set of inequalities is added, and each set of inequalities thus obtained only depends on the ordering of the switching fields. Hence, any transition $S \rightarrow S^{(1)} \rightarrow ... \rightarrow S^{(l)}$, of any length $l$, is fully determined from the ordering of the switching fields.\\
Finally, we state that the full transition graph is described by a set of inequalities that is simply a combination of the conditions on the individual transitions.(...)We conclude that the corresponding transition graph for a set of switching fields only depends on the ordering of the switching fields, as described by a set of linear inequalities. We refer to the set of inequalities underlying a graph as the design inequalities\cite{vhecke2021}.\\
...
Naively, one can construct all possible transition graphs by finding the possible transition paths for each individual transition. For example, for $N=2$ one finds $3^4\times 6^2 = 2916$ graphs (Fig.\ref{fig:transition_paths_N2}).\\
\begin{figure}[ht!]
    \centering
...
\node[squarednode](scrambling)[right=of mainloop] {Scrambling};
\node[squarednode](avalanches)[right=of scrambling] {Avalanches};
\node[squarednode](finalgraph)[right=of avalanches] {Final graph};
