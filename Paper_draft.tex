...
Naively, one can construct all possible transition graphs by finding the possible transition paths for each individual transition, and evaluating all possible combinations of transition paths. However, the naive method generates many transition graphs that are not realizable. For example, for $N=2$ one finds $3^5\times 6 = 1458$ graphs (Fig.\ref{fig:transition_paths_N2}), while the number of solvable graphs is only thirteen for linear coupling.\\
\begin{figure}[ht!]
    \centering
