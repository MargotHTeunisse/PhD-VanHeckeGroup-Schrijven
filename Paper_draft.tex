...
Indeed there are $N!$ Preisach graphs for a collection of $N$ hysterons, each corresponding to a different ordering of the down switching fields. The ordering for each graph can be easily identified by considering the main loop: that is, the sequence of down transitions connecting the state with all $s_i = 1$ to that with all $s_i=0$. We accordingly use the main loop as an identifier for the Preisach graphs. For instance, for $N=2$ there are two Preisach graphs, corresponding to the main loops (1, 2) and (2, 1).\\ 
