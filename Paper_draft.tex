...
\end{figure}
We now describe how the transition graph is constructed for a given set of switching fields $U_i^{+, -}(S)$.\\
The possible transitions from a stable state $S$ of the system are found in two steps. First, we vary the external driving $U$ until it reaches a value where a hysteron flips, which we will call $U^*$. There are two possible transitions at each state, corresponding to an increase or decrease in the driving $U$. For an increase in the driving $U$, the first hysteron to flip is that with the lowest up switching field, which we denote as $U^+(S)$:
\begin{equation}\label{eq:globalup}
U^* = U^+(S) = \min_{s_j = 0}U_j^+(S)
\end{equation}
For a decrease in the driving $U$, the first hysteron to flip is that with the highest down switching field, which we call $U^-(S)$:
\begin{equation}\label{eq:globaldown}
U^* = U^-(S) = \max_{s_j = 1}U_j^+(S)
\end{equation}\\
After a hysteron is flipped, we let the system relax by holding the driving at $U^*$ until a new stable state is reached. Stability under the field $U^*$ means that all hysterons in phase $s_i=0$ have up switching fields higher than $U^*$, while all hysterons in phase $s_i = 1$ have down switching fields lower than $U^*$. By applying the two steps of initiation and stability to all stable states, we find the entire transition graph. \\
We now consider how the values of the switching fields can influence the transition graph at each of the two steps. For the iniation, we only compare up switching fields with up switching fields, and down switching fields with down switching fields, at the same state $S$. At the second step, we compare both up switching fields and down switching fields with the current value of the driving, which is the field $U^*$ which initiates a transition. However, $U^*$ itself given by Equations \ref{eq:globalup} and \ref{eq:globaldown}, and thus equal to one of the switching fields $U_i^{+, -}(S)$. We remark that for both steps the transition graph only depends on a set of linear inequalities $U_i(S_A) > U_j(S_B)$. It follows that the transition graph is purely determined by the order of switching fields, rather than their absolute value.\\
In the case where hysterons are non-interacting, there are only $2N$ switching fields $\{u_i^{+, -}\}$. In addition, because we never compare an up switching field with a down switching fields in the absence of avalanches, the only orderings that can change the graph are the partial orderings of the up switching fields $\{u_i^+\}$ and of the down switching fields $\{u_i^-\}$. The number of orderings that can lead to a unique graph is then $N!\times N!$. When we fix the up switching fields to account for exchange symmetry, we find that the number of permutations is only $N!$.\\
Indeed there are $N!$ Preisach graphs for a collection of $N$ hysterons, each corresponding to a different ordering of the down switching fields\cite{terzimungan2020}. The ordering for each graph can be easily identified by considering the main loop: that is, the sequence of down transitions connecting the state with all $s_i = 1$ to that with all $s_i=0$. We accordingly use the main loop as an identifier for the Preisach graphs. For instance, for $N=2$ there are two Preisach graphs, corresponding to the main loops (1, 2) and (2, 1) \cite{terzimungan2020}.\\ 
...
We thus see that it is inefficient to find all graphs for an interacting system of hysterons by considering the permutations of all switching fields. Even so, we can make use of the fact that the state transition graph only depends on a partial ordering of the switching fields. Namely, we know that each graph has an underlying set of linear inequalities $U_i(S_A) > U_j(S_B)$, which we refer to as the design inequalities\cite{vhecke2021}. The solution space of the design inequalities provides the region in parameter space that leads to the given graph. In particular, if the design inequalities have no solution, then the given graph is not realizable.\\
