...
\title{Methods for constructing the transition graphs of an interacting hysteron model}
...
\end{figure}\\
A caveat to the base graph argument is that, within an avalanche, it may be possible for multiple hysterons to become unstable at once. In addition, it is conceivable that the switching field for a real system depends not only on the state $S$ but also on the driving field $U$, since the physical state of the system is different for different values of $U$. We choose to confine ourselves to the part of design space where only one hysteron can become unstable at a time and the switching fields only depend on $S$, so that the the concept of base graphs holds.\\
...
Scrambling entails that the ordering of switching fields can change throughout the graph. In order to discuss scrambling more explicitly, we here define the concept of the main loop of a transition graph. We define the up and down boundaries of the main loop as the series of up transitions going from the all-zero to the all-one state, and the series of down transitions going from the all-one to the all-zero state, respectively. In general, we account for exchange symmetry by fixing the ordering of the up boundary. Therefore, we often refer to the down boundary of the main loop as simply the main loop.\\
In the Preisach model, the ordering of the main loop fixes the transitions throughout the transition graph, so that each main loop corresponds to a unique Preisach graph. Because the down boundary of the main loop has $N!$ permutations, there are $N!$ Preisach graphs for a system of $N$ hysterons.\\
